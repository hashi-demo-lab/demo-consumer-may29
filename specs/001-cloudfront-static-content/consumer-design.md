...
|--------|------|--------|-------------|
| `bucket_name` | `string` | `module.s3_bucket.s3_bucket_id` | Origin S3 bucket name (used for object uploads and ARN construction). |
...
- [ ] **E: CloudWatch alarms module calls** — Add `module.alarm_5xx` and `module.alarm_4xx` in `main.tf`, both with `providers = { aws = aws.us_east_1 }`. Shared inputs: `comparison_operator = "GreaterThanThreshold"`, `evaluation_periods = var.alarm_evaluation_periods`, `period = var.alarm_period_seconds`, `statistic = "Average"`, `unit = "Percent"`, `namespace = "AWS/CloudFront"`, `dimensions = { DistributionId = module.cloudfront.cloudfront_distribution_id, Region = "Global" }`, `treat_missing_data = "notBreaching"`, `actions_enabled = true`. `alarm_5xx`: `alarm_name = "${var.name_prefix}-cdn-5xx-error-rate"`, `metric_name = "5xxErrorRate"`, `threshold = var.alarm_5xx_threshold`, `alarm_description = "CloudFront 5xx error rate exceeded ${var.alarm_5xx_threshold}% over ${var.alarm_evaluation_periods} periods"`. `alarm_4xx`: `alarm_name = "${var.name_prefix}-cdn-4xx-error-rate"`, `metric_name = "4xxErrorRate"`, `threshold = var.alarm_4xx_threshold`, `alarm_description = "CloudFront 4xx error rate exceeded ${var.alarm_4xx_threshold}% over ${var.alarm_evaluation_periods} periods"`. `alarm_actions` / `ok_actions` left as empty lists (out-of-scope SNS).
  Files: `main.tf` (modified).

- [ ] **F: Outputs** — Populate `outputs.tf` with every output from the §3 Outputs table: `bucket_name`, `bucket_arn`, `bucket_regional_domain_name`, `distribution_id`, `distribution_arn`, `distribution_domain_name`, `distribution_hosted_zone_id`, `alarm_5xx_arn`, `alarm_4xx_arn`, and the convenience `alarm_arns` list. Each output gets a `description` per constitution §2.4. None are sensitive.
  Files: `outputs.tf` (created).

- [ ] **G: Polish — README and example tfvars** — Create `README.md` documenting the module composition (S3 origin -> CloudFront OAC -> CloudWatch alarms), an inputs/outputs table mirroring §3, a deployment-instructions section (`terraform login`, workspace creation prerequisites, `terraform init`, `terraform plan`, expected smoke test against `distribution_domain_name`), and a small ASCII composition diagram. Create `terraform.auto.tfvars.example` with sample values for every non-required variable. Run `terraform fmt -recursive` and `terraform validate`.
  Files: `README.md` (created), `terraform.auto.tfvars.example` (created).
